    ...
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    // 將數值存入以方便計算SQNR
                    unquantizedValues->Y[m][n][i * DCT_W + j] = quantizedValues->Y[m][n][i * DCT_W + j];
                    unquantizedValues->Cb[m][n][i * DCT_W + j] = quantizedValues->Cb[m][n][i * DCT_W + j];
                    unquantizedValues->Cr[m][n][i * DCT_W + j] = quantizedValues->Cr[m][n][i * DCT_W + j];
                }
            }
        }
    }
        ...
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                for (int j = 0; j < DCT_W; j++)
                {
                    ...
                    {
                    case 0: // Y component
                        data = (short)F->Y[m][n][i * DCT_W + j];
                        break;
                    case 1: // Cb component
                        data = (short)F->Cb[m][n][i * DCT_W + j];
                        break;
                    case 2: // Cr component
                        data = (short)F->Cr[m][n][i * DCT_W + j];
                        break;
                    }
                    // 將數據以二進位形式寫入文件
