    ...
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    int u = i * DCT_W + j; // DCT coefficient index
                    float value_Y = (float)errors->Y[m][n][u];
                    float value_Cb = (float)errors->Cb[m][n][u];
                    float value_Cr = (float)errors->Cr[m][n][u];

                    // 將誤差值寫入對應的文件
                    fwrite(&value_Y, sizeof(float), 1, file_Y);
                    fwrite(&value_Cb, sizeof(float), 1, file_Cb);
                    fwrite(&value_Cr, sizeof(float), 1, file_Cr);
                }
            }
        }
    }
