...
// RGB to YCbCr轉換
void convertRGBtoYCbCr(ImgRGB **rgbData, ImgYCbCr **ycbcrData, int H, int W)
{
    // 遍歷圖像的每一個像素
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            ycbcrData[i][j].Y = round(0.299 * R + 0.587 * G + 0.114 * B);
            ycbcrData[i][j].Cb = round(128 - 0.168736 * R - 0.331264 * G + 0.5 * B);
            ycbcrData[i][j].Cr = round(128 + 0.5 * R - 0.418688 * G - 0.081312 * B);
        }
    }
}
...
void allocateDCTArray(DCTArray *F, int M, int N)
{
    ...
    F->R = (double ***)malloc(M * sizeof(double **));
    F->G = (double ***)malloc(M * sizeof(double **));
    F->B = (double ***)malloc(M * sizeof(double **));
    for (int i = 0; i < M; i++)
    {
        // 針對每個顏色通道，進一步分配第二層指標
        F->R[i] = (double **)malloc(N * sizeof(double *));
        F->G[i] = (double **)malloc(N * sizeof(double *));
        F->B[i] = (double **)malloc(N * sizeof(double *));
        
        // 對於每個元素，分配足夠的空間來存儲DCT（離散餘弦變換）轉換後的數據
        for (int j = 0; j < N; j++)
        {
            F->R[i][j] = (double *)malloc(DCT_H * DCT_W * sizeof(double));
            F->G[i][j] = (double *)malloc(DCT_H * DCT_W * sizeof(double));
            F->B[i][j] = (double *)malloc(DCT_H * DCT_W * sizeof(double));
        }
    }
}
        ...
        for (int v = 0; v < DCT_W; v++)
        {
            ...
            for (int r = 0; r < DCT_H; r++)
            {
                for (int c = 0; c < DCT_W; c++)
                {
                    ...
                    // 將計算出的值賦予基向量陣列的對應元素
                    basis_vector[r][c][u][v].R = cosR * cosC;
                    basis_vector[r][c][u][v].G = cosR * cosC;
                    basis_vector[r][c][u][v].B = cosR * cosC;
                }
            }
        }
    }
...
// 執行2D-DCT
void perform2DDCT(ImgYCbCr **image, DCTArray *F, int H, int W, int DCT_M, int DCT_N)
{
                ...
                for (int v = 0; v < DCT_W; v++)
                {
                    ...
                    for (int x = 0; x < DCT_H; x++)
                    {
                        for (int y = 0; y < DCT_W; y++)
                        {
                            ...
                            // 將圖像數據與基向量相乘並累加
                            sumY += (pixel.Y - 128.0) * basis_vector[x][y][u][v].R;
                            sumCb += (pixel.Cb - 128.0) * basis_vector[x][y][u][v].G;
                            sumCr += (pixel.Cr - 128.0) * basis_vector[x][y][u][v].B;
                        }
                    }
                    // 根據DCT公式計算並存儲轉換後的數據
                    F->R[m][n][u * DCT_W + v] = 2.0 / sqrt(DCT_H * DCT_W) * beta[u] * beta[v] * sumY;
                    F->G[m][n][u * DCT_W + v] = 2.0 / sqrt(DCT_H * DCT_W) * beta[u] * beta[v] * sumCb;
                    F->B[m][n][u * DCT_W + v] = 2.0 / sqrt(DCT_H * DCT_W) * beta[u] * beta[v] * sumCr;
                }
            }
        }
    }
...
// 複製2D-DCT後的數值
void copyDCTValues(DCTArray *quantizedValues, DCTCoefficients *unquantizedValues, int DCT_M, int DCT_N)
{
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    // 將數值存入以方便計算SQNR
                    unquantizedValues->Y[m][n][i * DCT_W + j] = quantizedValues->R[m][n][i * DCT_W + j];
                    unquantizedValues->Cb[m][n][i * DCT_W + j] = quantizedValues->G[m][n][i * DCT_W + j];
                    unquantizedValues->Cr[m][n][i * DCT_W + j] = quantizedValues->B[m][n][i * DCT_W + j];
                }
            }
        }
    }
    ...
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            for (int u = 0; u < DCT_H; u++)
            {
                for (int v = 0; v < DCT_W; v++)
                {
                    F->R[m][n][u * DCT_W + v] = round(F->R[m][n][u * DCT_W + v] / Q_Y[u][v]);
                    F->G[m][n][u * DCT_W + v] = round(F->G[m][n][u * DCT_W + v] / Q_Cb[u][v]);
                    F->B[m][n][u * DCT_W + v] = round(F->B[m][n][u * DCT_W + v] / Q_Cr[u][v]);
                }
            }
        }
    }
        ...
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                for (int j = 0; j < DCT_W; j++)
                {
                    ...
                    {
                    case 0: // Y component
                        data = (short)F->R[m][n][i * DCT_W + j];
                        break;
                    case 1: // Cb component
                        data = (short)F->G[m][n][i * DCT_W + j];
                        break;
                    case 2: // Cr component
                        data = (short)F->B[m][n][i * DCT_W + j];
                        break;
                    }
                    // 將數據以二進位形式寫入文件
        ...
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                for (int j = 0; j < DCT_W; j++)
                {
                    ...
                    // Y channel error
                    unquantizedCoeffs->Y[m][n][u] -= quantizedCoeffs->R[m][n][u] * Q_Y[i][j];
                    // Cb channel error
                    unquantizedCoeffs->Cb[m][n][u] -= quantizedCoeffs->G[m][n][u] * Q_Cb[i][j];
                    // Cr channel error
                    unquantizedCoeffs->Cr[m][n][u] -= quantizedCoeffs->B[m][n][u] * Q_Cr[i][j];
                }
            }
        }
    }
    ...
    for (int m = 0; m < DCT_M; m++)
    {
        for (int n = 0; n < DCT_N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    int u = i * DCT_W + j; // DCT coefficient index
                    short value_Y = (short)errors->Y[m][n][u];
                    short value_Cb = (short)errors->Cb[m][n][u];
                    short value_Cr = (short)errors->Cr[m][n][u];

                    // 將誤差值寫入對應的文件
                    fwrite(&value_Y, sizeof(short), 1, file_Y);
                    fwrite(&value_Cb, sizeof(short), 1, file_Cb);
                    fwrite(&value_Cr, sizeof(short), 1, file_Cr);
                }
            }
        }
    }
        ...
        for (int j = 0; j < DCT_W; j++)
        {
            ...
            for (int m = 0; m < DCT_M; m++)
            {
                for (int n = 0; n < DCT_N; n++)
                {
                    ...
                    double noise_Y = signal_Y - (quantized->R[m][n][u] * Q_Y[i][j]);

                    double signal_Cb = original->Cb[m][n][u];
                    double noise_Cb = signal_Cb - (quantized->G[m][n][u] * Q_Cb[i][j]);

                    double signal_Cr = original->Cr[m][n][u];
                    double noise_Cr = signal_Cr - (quantized->B[m][n][u] * Q_Cr[i][j]);
...
int main(int argc, char **argv)
{
    ...
    else if (mode == 1)
    {
        ...
        ImgRGB **Data_RGB = malloc_2D(H, W);
        ImgYCbCr **Data_YCbCr = malloc_2D_YCbCr(H, W);
        ...
        convertRGBtoYCbCr(Data_RGB, Data_YCbCr, H, W);
        ...
        // 進行 2D DCT 轉換
        perform2DDCT(Data_YCbCr, &F, H, W, DCT_M, DCT_N);
