...
// 將量化誤差加到量化數據上
void addQuantizationError(short ***quantizedData, float ***errorData, int quantizationTable[DCT_H][DCT_W], int height, int width)
{
    int M = height / DCT_H;
    int N = width / DCT_W;
    ...
    for (int m = 0; m < M; m++)
    {
        for (int n = 0; n < N; n++)
        {
            for (int i = 0; i < DCT_H; i++)
            {
                ...
                {
                    int index = i * DCT_W + j;
                    quantizedData[m][n][index] = quantizedData[m][n][index] * quantizationTable[i][j] + errorData[m][n][index];
                }
            }
        }
    }
...
// 反2D-DCT
// 反2D-DCT
void inverseDCT(short ***quantizedData, double **image, double basis_vector[DCT_H][DCT_W][DCT_H][DCT_W], int height, int width)
{
...
int main(int argc, char *argv[])
{
    ...
    // 將量化誤差加到量化數據上
    addQuantizationError(quantized_Y, error_Y, Q_Y, height, width);
    addQuantizationError(quantized_Cb, error_Cb, Q_Cb, height, width);
    addQuantizationError(quantized_Cr, error_Cr, Q_Cr, height, width);
    ...
    // 反 2D-DCT
    inverseDCT(quantized_Y, image_Y, basis_vector, height, width);
    inverseDCT(quantized_Cb, image_Cb, basis_vector, height, width);
    inverseDCT(quantized_Cr, image_Cr, basis_vector, height, width);
