...
// 反2D-DCT並加上誤差值
void inverseDCTWithCombinedError(short ***quantizedData, float ***errorData, double **image, int quantizationTable[DCT_H][DCT_W], double basis_vector[DCT_H][DCT_W][DCT_H][DCT_W], int height, int width)
{
    int M = height / DCT_H;
    int N = width / DCT_W;
    ...
    for (int m = 0; m < M; m++)
    {
        for (int n = 0; n < N; n++)
        {
            for (int x = 0; x < DCT_H; x++)
            {
                ...
                {
                    // Add the error data to the quantized data before IDCT
                    quantizedData[m][n][x * DCT_W + y] += errorData[m][n][x * DCT_W + y];
...
int main(int argc, char *argv[])
{
    ...
    // 反 2D-DCT並加上誤差值
    inverseDCTWithCombinedError(quantized_Y, error_Y, image_Y, Q_Y, basis_vector, height, width);
    inverseDCTWithCombinedError(quantized_Cb, error_Cb, image_Cb, Q_Cb, basis_vector, height, width);
    inverseDCTWithCombinedError(quantized_Cr, error_Cr, image_Cr, Q_Cr, basis_vector, height, width);
