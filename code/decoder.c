...
// 反2D-DCT
void inverseDCT(short ***quantizedData, double **image, int quantizationTable[DCT_H][DCT_W], double basis_vector[DCT_H][DCT_W][DCT_H][DCT_W], int height, int width)
{
...
int main(int argc, char *argv[])
{
    ...
    if (command == 1)
    {
        ...
        // 反 2D-DCT
        inverseDCT(quantized_Y, image_Y, Q_Y, basis_vector, height, width);
        inverseDCT(quantized_Cb, image_Cb, Q_Cb, basis_vector, height, width);
        inverseDCT(quantized_Cr, image_Cr, Q_Cr, basis_vector, height, width);
